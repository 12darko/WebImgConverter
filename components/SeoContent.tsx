...
seoData.tr['favicon-generator'] = { title: 'Favicon Oluşturucu: Web Siteniz İçin Profesyonel İkonlar', intro: 'WebImgConverter Favicon Oluşturucu ile PNG, JPG veya SVG görsellerinizi saniyeler içinde tüm tarayıcılar ile uyumlu .ico formatına dönüştürün.', whyTitle: 'Neden Favicon Kullanmalısınız?', features: [ { title: 'Marka Bilinirliği', desc: 'Ziyaretçileriniz sitenizi logonuz sayesinde kolayca bulur.' } ], faq: [ { q: 'Hangi formatları yükleyebilirim?', a: 'PNG, JPG ve SVG formatlarındaki görsellerinizi yükleyebilirsiniz.' } ] }; seoData.en['favicon-generator'] = seoData.tr['favicon-generator']; seoData.de['favicon-generator'] = seoData.tr['favicon-generator']; seoData.fr['favicon-generator'] = seoData.tr['favicon-generator']; seoData.tr['watermark-image'] = { title: 'Fotoğraflara Filigran (Watermark) Ekleme', intro: 'Görsellerinizin izinsiz kullanılmasını önlemek veya marka kimliğinizi vurgulamak için fotoğraflarınıza kolayca metin veya logo filigranı ekleyin.', whyTitle: 'Neden Filigran Eklemelisiniz?', features: [ { title: 'Telif Hakkı Koruması', desc: 'Eserlerinizi çalanların işini zorlaştırın.' } ], faq: [ { q: 'Kalite bozulur mu?', a: 'Hayır, fotoğraflarınız orijinal kalitesinde kalır.' } ] }; seoData.en['watermark-image'] = seoData.tr['watermark-image']; seoData.de['watermark-image'] = seoData.tr['watermark-image']; seoData.fr['watermark-image'] = seoData.tr['watermark-image']; seoData.tr['rotate-image'] = { title: 'Resim Döndürme: Görsellerinizi İstediğiniz Açıya Getirin', intro: 'Ters çekilmiş veya yan duran fotoğraflarınızı tek tıkla 90 derece sağa/sola döndürün.', whyTitle: 'Resimleri Neden Döndürmelisiniz?', features: [ { title: 'Hızlı Düzeltme', desc: 'Hatalı algılanan yönleri anında düzeltin.' } ], faq: [ { q: 'Boyut değişir mi?', a: 'Hayır, kalite ve boyut değişmez.' } ] }; seoData.en['rotate-image'] = seoData.tr['rotate-image']; seoData.de['rotate-image'] = seoData.tr['rotate-image']; seoData.fr['rotate-image'] = seoData.tr['rotate-image']; seoData.tr['black-and-white'] = { title: 'Siyah Beyaz Fotoğraf Yapma: Görsellerinize Nostaljik Bir Hava Katın', intro: 'Renkli fotoğraflarınızı tek tıkla yüksek kaliteli siyah beyaz görsellere dönüştürün.', whyTitle: 'Neden Siyah Beyaz?', features: [ { title: 'Odaklanma', desc: 'Renklerin dikkat dağıtıcı etkisinden kurtulup ana konuya odaklanın.' } ], faq: [ { q: 'Geri alınabilir mi?', a: 'Araç orijinal dosyaya dokunmaz, yeni bir dosya indirirsiniz.' } ] }; seoData.en['black-and-white'] = seoData.tr['black-and-white']; seoData.de['black-and-white'] = seoData.tr['black-and-white']; seoData.fr['black-and-white'] = seoData.tr['black-and-white']; seoData.tr['remove-background'] = { title: 'Yapay Zeka ile Arka Plan Silme: Fotoğraflarınızı Şeffaflaştırın', intro: 'Yapay zeka gücüyle saniyeler içinde fotoğraflarınızın arka planını temizleyin.', whyTitle: 'Neden AI Arka Plan Silme?', features: [ { title: 'Kusursuz Detaylar', desc: 'Saç telleri ve kürk ayrıştırılır.' } ], faq: [ { q: 'Hangi formatlar?', a: 'JPG, PNG, WebP ve AVIF.' } ] }; seoData.en['remove-background'] = seoData.tr['remove-background']; seoData.de['remove-background'] = seoData.tr['remove-background']; seoData.fr['remove-background'] = seoData.tr['remove-background']; seoData.tr['crop'] = { title: 'Akıllı Resim Kırpma: Görsellerinizi Mükemmel Oranlara Getirin', intro: 'Fotoğraflarınızı platformların istediği boyutlara kolayca kırpın.', whyTitle: 'Neden Kırpma?', features: [ { title: 'Mükemmel Odak', desc: 'Gereksiz kenarları atın.' } ], faq: [ { q: 'Hangi oranlar?', a: '1:1, 4:3, 16:9 vs.' } ] }; seoData.en['crop'] = seoData.tr['crop']; seoData.de['crop'] = seoData.tr['crop']; seoData.fr['crop'] = seoData.tr['crop']; seoData.tr['compress-image'] = { title: 'Görsel Sıkıştırma: Dosya Boyutunu Küçültün', intro: 'Dosyalarınızı kalite kaybı olmadan sıkıştırın.', whyTitle: 'Neden Sıkıştırma?', features: [ { title: 'Hızlı Sayfalar', desc: 'Web hızınızı artırın.' } ], faq: [ { q: 'Kalite düşer mi?', a: 'Gözle görülür bozulma olmaz.' } ] }; seoData.en['compress-image'] = seoData.tr['compress-image']; seoData.de['compress-image'] = seoData.tr['compress-image']; seoData.fr['compress-image'] = seoData.tr['compress-image'];
