...
export default function ProfilePage() {
    ...
    return (
        <SiteShell>
            <Helmet>
                <title>Hesabım | WebImgConverter</title>
                <meta name="description" content="Hesap bilgilerinizi ve üyelik detaylarınızı yönetin." />
                ...
                <div className="max-w-3xl mx-auto px-5 md:px-8">
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm">
                        ...
                        <div className="bg-slate-950 px-8 py-10 text-white relative overflow-hidden">
                            ...
                            <h1 className="text-3xl font-bold font-serif mb-2 relative z-10">Hesabım</h1>
                            <p className="text-slate-300 dark:text-slate-400 text-sm relative z-10">Üyelik detaylarınızı ve kullanım istatistiklerinizi buradan yönetebilirsiniz.</p>
                        </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div>
                                    ...
                                    <div className="bg-slate-50 dark:bg-slate-950/40 rounded-2xl p-5 border border-slate-100 dark:border-slate-800/60 space-y-4">
                                        <div>
                                            <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">E-Posta</label>
                                            <p className="text-slate-900 dark:text-white font-medium">{user?.email}</p>
                                        </div>
                                        <div>
                                            <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Hesap ID</label>
                                ...
                                <div>
                                    ...
                                    <div className="bg-brand-50/50 dark:bg-brand-950/10 rounded-2xl p-5 border border-brand-100/50 dark:border-brand-900/30 space-y-4">
                                        <div>
                                            <label className="block text-xs font-semibold text-brand-600/70 dark:text-brand-400/80 uppercase tracking-wider mb-1">Mevcut Plan</label>
                                            <div className="flex items-center gap-2">
                                                <span className="text-brand-900 dark:text-brand-400 font-bold uppercase text-lg">{stats.premiumTier}</span>
                                                {stats.isPremium && (
                                                    <span className="bg-brand-100 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase">Aktif</span>
                                                )}
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-2 gap-4 pt-2">
                                            <div>
                                                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Kalan Kredi</label>
                                                <p className="text-2xl font-bold text-slate-900 dark:text-white">{stats.premiumTier === 'business' ? '∞' : stats.credits}</p>
                                            </div>
                                            <div>
                                                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Toplam İşlem</label>
